...
pacman::p_load(readr, dplyr, haven, survival)

# Load data
epivigila <- read_dta("~/Dropbox/Data MINSAL/data/20200712_informe_epi_total(resumen).dta")

epivigila$fecha_reporte <- lubridate::today()
epivigila$estado <- ifelse((epivigila$fecha_reporte-epivigila$fecha_primeros_sintomas)>40,2,
                           ifelse(epivigila$estado_paciente=="fallecido",1,0))
# epivigila$fallecido <- ifelse(epivigila$estado_paciente=="fallecido",1,0)
# epivigila$recuperado <- ifelse(epivigila$estado_paciente=="fallecido",1,0)

# Example
casefat(t=as.numeric(epivigila$fecha_primeros_sintomas),f=epivigila$estado)